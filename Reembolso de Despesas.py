...
with aba_solicitacao:
    ...
    # O formulário envolve tudo para garantir o reset total
    with st.form("meu_formulario", clear_on_submit=True):
            ...
            for cat in selecionadas:
                with st.container():
                    ...
                    if "KM (em qtde)" in cat:
                        q_km = c3.number_input("Qtde KM", min_value=0.0, step=0.1, key=f"v_{cat}")
                        v_fin = (q_km * 1.37) if q_km else 0.0
                        if q_km: c3.info(f"R$ {v_fin:.2f}")
                    else:
                        v_fin = c3.number_input("Valor R$", min_value=0.0, step=0.01, key=f"v_{cat}")
        ...
        if enviar:
            if any(not d["Motivo"].strip() for d in dados_despesas) or not arq or not nome:
                st.error("Preencha o nome, todos os motivos e anexe os arquivos.")
            else:
                ...
                st.warning("Os campos foram limpos para uma nova solicitação.")
                time.sleep(2)
                st.rerun()
